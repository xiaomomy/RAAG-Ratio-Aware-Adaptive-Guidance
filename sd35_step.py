    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            while i < len(timesteps):          
                ...
                # perform guidance
                if self.do_classifier_free_guidance or choice:
                    ...
                    if choice:
                        ...
                        ratio = torch.norm(delta) / torch.norm(uncond)

                        w = 1 + (max_guidance - 1) * torch.exp(-lr_para * ratio)
                        
                        noise_pred = noise_pred_uncond + w * (noise_pred_text - noise_pred_uncond)
                        print(f"Step {i} Guidance Scale: {w}.")
                ...
                latents = self.scheduler.step(noise_pred, t, latents, return_dict=False)[0]# if i > 0 else latents
